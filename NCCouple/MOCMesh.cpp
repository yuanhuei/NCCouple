		...
		for (int i = 0; i < layerMeshNum; i++)
		{
			...
			if(kernelType == MeshKernelType::CGAL_KERNEL)
				...
				m_meshPointPtrVec[index] = std::make_shared<LingMocMeshPoint>(
					meshIDtemp_, fileNameTemperary[index], allEdges[i].edgeType, allMeshFaces[i].faceEdges[j], allEdges[i].arcAxisDir,
...
void MOCMesh::setEdgeInformation(string lineType, string linePosition, int edgeIDTemperary, std::vector<Edge>& allEdges, int nFineMesh)
{
	...
	if (edgeTypeTemperary == 3)//arc
	{
		...
		for (int i = 1; i <= nFineMesh; i++)
		{
			...
			std::array<double, 3> arcCenter_{ Center_point_X, Center_point_Y, 0.0 };
			std::array<double, 3> arcAxisDir_{ 0.0, 0.0, 1.0 };
			edge0.arcCenter.push_back(arcCenter_);
			edge0.arcAxisDir.push_back(arcAxisDir_);
			allEdges.push_back(edge0);  //conserve edge object
...
{
	double x0 = 0.63, y0 = 0.63;
	double dDistance = sqrt((x - x0) * (x - x0) + (y - y0) * (y - y0));//和圆柱中心点的距离
	double dAngletoX = atan2(y - y0, x - x0) * 180 / PI;//计算和X轴夹角
	if (dAngletoX < 0)
		dAngletoX = 360 + dAngletoX;//x轴下，夹角是负的，需要转换成正的
	int iNum0f_Rdirect = 7, iNumof_Cirdirection = 8;//径向和周向划分区段
	double dR1 = 0.1024, dR2 = 0.2048, dR3 = 0.3072, dR4 = 0.409, dR5 = 0.418, dR6 = 0.475, dR7 = 0.63;

	int iMeshIndex = 0;
	double dAngel = 360 / iNumof_Cirdirection;
	//计算dAngletoX >= 0 && dAngletoX < dAngel下的index
	if (dDistance >= 0 && dDistance < dR1)
		iMeshIndex = 0;
	else if (dDistance >= dR1 && dDistance < dR2)
		iMeshIndex = iNumof_Cirdirection;
	else if (dDistance >= dR2 && dDistance < dR3)
		iMeshIndex = 2 * iNumof_Cirdirection;
	else if (dDistance >= dR3 && dDistance < dR4)
	{
		if (dDistance < 0.39)
			iMeshIndex = 3 * iNumof_Cirdirection;
		else
			iMeshIndex = 4 * iNumof_Cirdirection;
	}
	else if (dDistance >= dR4 && dDistance < dR5)
		iMeshIndex = 4 * iNumof_Cirdirection;
	else if (dDistance >= dR5 && dDistance < dR6)
		iMeshIndex = 5 * iNumof_Cirdirection;
	else if (dDistance >= dR6 && dDistance <= dR7)
		iMeshIndex = 6 * iNumof_Cirdirection;
	iMeshIndex = iMeshIndex + int(dAngletoX / dAngel);

	Logger::LogInfo(FormatStr("dDistance is : %f,angelNum: %d", dDistance, int(dAngletoX / dAngel)));
	return iMeshIndex;
}//输入CFD中心点坐标，计算出所属网格的编号
...
void MOCMesh::reOrganaziIndex()
{
	...
	for (int i = 0; i < m_meshPointPtrVec_copy.size(); i++)
	{
		double x = std::get<0>(m_meshPointPtrVec_copy[i]->CentralCoordinate());
		double y = std::get<1>(m_meshPointPtrVec_copy[i]->CentralCoordinate());
		Logger::LogInfo(FormatStr("x is : %f,y is : %f", x - 0.63, y - 0.63));
		int index = CalMeshIndex(x, y);
		Logger::LogInfo(FormatStr("i is : %d,index recaculate: %d \n", i, index));
